...
const RANDOM_UPDATE_INTERVAL = 200; // MUITO MAIS RÁPIDO
...
function init() {
    ...
    function animate() {
        ...
        if (horseModel) {
            ...
            flipAngle -= flipSpeed * (0.7 + Math.random() * 0.6);
            ...
            const microVibration = Math.sin(currentTime * 0.08) * 0.3 * chaosLevel + 
                                  Math.sin(currentTime * 0.12) * 0.15 * chaosLevel +
                                  Math.sin(currentTime * 0.2) * 0.08 * chaosLevel;
            
            swayTime += 0.12;
            sideSway = Math.sin(swayTime * 3.2) * 0.4 * chaosLevel + 
                      Math.sin(swayTime * 1.8) * 0.2 * chaosLevel;
...
function updateRandomness() {
    randomOffsetX = (Math.random() - 0.5) * 0.5 * chaosLevel;
    randomOffsetY = (Math.random() - 0.5) * 0.4 * chaosLevel;
    randomOffsetZ = (Math.random() - 0.5) * 0.25 * chaosLevel;
    
    targetJitter = (Math.random() - 0.5) * 1.2 * chaosLevel;
    
    tiltVariation = (Math.random() - 0.5) * 0.3 * chaosLevel;
}
...
function applyControlledChaos(currentTime) {
    ...
    horseModel.rotation.z += Math.sin(time * 12.5) * 0.1 * chaosLevel +
                           ...
                           Math.sin(time * 29.3) * 0.05 * chaosLevel +
                           randomJitter * 0.8;
    
    horseModel.rotation.y += Math.sin(time * 6.9) * 0.03 * chaosLevel;
    
    const headTilt = Math.sin(time * 18.2) * 0.2 * chaosLevel +
                    Math.sin(time * 25.1) * 0.12 * chaosLevel;
    horseModel.rotation.x += headTilt;
    horseModel.rotation.z += headTilt * 0.9;
    
    const bodyVibration = Math.sin(time * 32.5) * 0.15 * chaosLevel +
                         Math.sin(time * 27.8) * 0.1 * chaosLevel +
                         Math.sin(time * 45.2) * 0.06 * chaosLevel;
    horseModel.rotation.x += bodyVibration;
    horseModel.rotation.y += bodyVibration * 0.5;
    horseModel.rotation.z += bodyVibration * 0.6;
    
    const wildMovement = Math.sin(time * 52.7) * 0.08 * chaosLevel +
                        Math.sin(time * 38.4) * 0.05 * chaosLevel;
    horseModel.rotation.x += wildMovement * 0.7;
    horseModel.rotation.z += wildMovement * 1.2;
    horseModel.rotation.y += wildMovement * 0.4;
}
